//
//  ModifiedWikiPayTableViewCell.m
//  ZXProject
//
...
#import "ModifiedWikiPayTableViewCell.h"
#import "CommonHeader.h"

@implementation ModifiedWikiPayTableViewCell
...
-(void)payAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(modifiedWikiPaySelect2go:)]) {
        [self.delegate modifiedWikiPaySelect2go:tag];
        NSLog(@"打赏");
    }
...
-(void)btnAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(modifiedWikiPayMemberSelect2go:)]) {
        [self.delegate modifiedWikiPayMemberSelect2go:tag];
        NSLog(@"点击头像");
    }
