...
#import "MiaoCirclePayTableViewCell.h"
#import "CommonHeader.h"

@implementation MiaoCirclePayTableViewCell
...
-(void)payAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(miaoCirclePaySelect2go:)]) {
        [self.delegate miaoCirclePaySelect2go:tag];
        NSLog(@"打赏");
    }
...
-(void)btnAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(miaoCirclePayMemberSelect2go:)]) {
        [self.delegate miaoCirclePayMemberSelect2go:tag];
        NSLog(@"点击头像");
    }
