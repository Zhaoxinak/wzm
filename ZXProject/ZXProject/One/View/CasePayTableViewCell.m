...
#import "CasePayTableViewCell.h"
#import "CommonHeader.h"

@implementation CasePayTableViewCell
...
-(void)payAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(casePaySelect2go:)]) {
        [self.delegate casePaySelect2go:tag];
        NSLog(@"打赏");
    }
...
-(void)btnAct:(UIButton *)button{
    ...
    if ([self.delegate respondsToSelector:@selector(casePayMemberSelect2go:)]) {
        [self.delegate casePayMemberSelect2go:tag];
        NSLog(@"点击头像");
    }
